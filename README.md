...
다음의 목차에 따라 진행하며 테스트 툴 조합 항목에서는 각 조합으로 카운터, 투두 리스트, 비동기 컴포넌트 테스트 케이스를 작성하겠습니다. 결론 부분에서는 최종적으로 어떤 툴을 사용하는 게 좋을지 도출하겠습니다.
## 목차
1. 용어설명
...
```
추가로 Jest에서 함수를 mocking하는 대표적인 4가지 함수를 알아보겠습니다.
1. ```jest.fn()```
...
저는 **Cypress** 입니다. 환경 설정이 까다롭다는 단점이 있지만 메소드 체이닝 패턴으로 이어가는 형식으로 코드가 간결했고 비동기 코드도 동기적으로 실행되어 순차적으로 코드를 읽으며 이해하기 어렵지 않았습니다. 그리고 지원되는 에드온으로 컴포넌트 테스트가 가능해 개발단부터 E2E테스트 까지(QA/QC) 어느정도 커버할 수 있습니다.
